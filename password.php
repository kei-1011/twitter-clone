<?php
  ...
  if(isset($_POST['submit'])) {
    ...
    $error        = [];
    if(isset($currentPwd) && isset($newPassword) && isset($rePassword)) {
      if($getFromU->checkPassword($currentPwd) === true) {

        if(strlen($newPassword) < 6 ) {
          $error['newPassword'] = 'パスワードは6文字以上で入力してください。';
        } else if($newPassword != $rePassword) {
          $error['rePassword'] = '確認用パスワードが正しくありません。';
        } else {
        ...
        $error['currentPwd'] = 'パスワードが正しくありません。';
      }

    } else {
      $error['fields'] = '全ての項目は必須です。';
    }
  }
?>
<html>
